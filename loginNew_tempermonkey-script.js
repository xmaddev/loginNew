...
(function() {
    ...
    document.body.insertAdjacentHTML( 'afterend', '<div class="video-yt-slider"></div>');
    ...
    document.querySelector('.video-yt-slider').addEventListener('click', () => sliderMemes());
    var sliderMemes = () => {
        GM_xmlhttpRequest({
        method: "GET",
...
/*     var getTypeRow = (el,type) => {
    ...
    var toggle = false;
    document.querySelector('.video-yt-slider').addEventListener('click',(e) => {toggle = !toggle; if(toggle) document.querySelector('.video-yt-slider>p+input.video-yt').style.cssText += 'display:block;'; else document.querySelector('.video-yt-slider>p+input.video-yt').style.cssText += 'display:none;';});
    ...
    document.getElementsByTagName('head')[0].insertAdjacentHTML( 'afterbegin','<style type="text/css">  body {overflow-x: hidden;}.video-container {width: 100vw;height: 100vh;overflow: hidden;position: absolute;}iframe {position: absolute;top: -10%;width: 100vw;height: 117vh;pointer-events: none;} .t-Region-body h1:after {content: "";display: block;width: 200px;height: 4px;background: linear-gradient(160deg, red 15%, white 20% 35%, red 40% 55%, white 60% 75%, red 80%);animation: lilipop-animated 1s linear infinite;background-size: 25%;}@keyframes lilipop-animated {100% {background-position: 100%;}}.t-Login-body p {cursor: pointer;margin: 2px 0;background: #221e1d;text-align: center;margin-bottom: 1,2rem;padding: 4px;color:#fff;}</style>');
    //chack class exist
    ...
    // Random change login button position
/*     var triesNum = 0;
    document.querySelector('.t-Login-region').addEventListener('mouseleave', () => {
        document.getElementById('B115197341225796797').style.cssText += 'position:relative;left:auto;top:auto';
         quotes.innerText = '';
    });
    document.getElementById('B115197341225796797').addEventListener('mouseover',(e) => {
        if( triesNum == 0)
        {
            e.target.style.cssText += `position:absolute;left:${Math.floor(Math.random() * (100 - (-100) + 1)) - 100}px;top:${Math.floor(Math.random() * 400)}px;`;
            quotes.innerText = 'Почти поймал';
            document.body.appendChild(quotes);
        }
        else if( triesNum == 1)
        {
            e.target.style.cssText += `position:absolute;left:${Math.floor(Math.random() * (100 - (-100) + 1)) - 100}px;top:${Math.floor(Math.random() * 400)}px;`;
            quotes.innerText = 'Уже близко';
            document.body.appendChild(quotes);
        }
        else if( triesNum == 2)
        {
            quotes.innerText = 'Ладно,ладно...:)';
            document.getElementById('B115197341225796797').style.cssText += 'position:relative;left:auto;top:auto';
        }
        triesNum++;
    }); */
    ...
    //document.body.style.background = 'url(https://github.com/xmaddev/loginNew/blob/main/When%20I%20just%20deleted%20all%20the%20users%20account%E2%80%A6%20in%20production.gif?raw=true) no-repeat center/cover';
